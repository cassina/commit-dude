...
def _build_messages(diff: str) -> List[BaseMessage]:
    return [
...
def generate_commit_message(diff: str) -> CommitMessageResponse:
    ...
    structured_llm = llm.with_structured_output(CommitMessageResponse)

    num_tokens = llm.get_num_tokens(diff)
    print(f"Num tokens: {num_tokens}")
    if num_tokens > MAX_TOKENS:
        raise ValueError(
            f"Diff is too long. Max tokens: {MAX_TOKENS}, diff tokens: {num_tokens}"
        )
